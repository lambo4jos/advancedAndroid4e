package com.advancedandroidbook.simpledatabase;
...
public class SimpleDatabaseActivity extends Activity {
	...
	// Using triggers to enforce foreign key constraint, because Sqlite doesnt
	// enforce them
	...
	public void runDatabaseExample() {
		...
		Log.i(DEBUG_TAG,
				"SQL QUERY EQUIVALENT: SELECT title, id  FROM tbl_books ORDER BY title ASC;");
		String asColumnsToReturn2[] = { "title", "id" };
		String strSortOrder2 = "title ASC";
		c = mDatabase.query("tbl_books", asColumnsToReturn2, null, null, null,
				null, strSortOrder2);
		LogCursorInfo(c);
		c.close();

		// JOIN QUERY USING SQLiteQueryBuilder: select title, author first name,
		// author last name, and ids (Join two tables)
		Log.i(DEBUG_TAG,
				"SQL QUERY EQUIVALENT: SELECT tbl_books.title, tbl_books.id, tbl_authors.firstname, tbl_authors.lastname, tbl_books.authorid FROM tbl_books INNER JOIN tbl_authors on tbl_books.authorid=tbl_authors.id ORDER BY title ASC;");
		SQLiteQueryBuilder queryBuilder = new SQLiteQueryBuilder();
		queryBuilder.setTables(TABLE_BOOK + ", " + TABLE_AUTHOR); // Tables to
																	// join
		queryBuilder.appendWhere(TABLE_BOOK + ".authorid" + "=" + TABLE_AUTHOR
				+ ".id"); // how to join
		String asColumnsToReturn[] = { TABLE_BOOK + ".title",
				TABLE_BOOK + ".id", TABLE_AUTHOR + ".firstname",
				TABLE_AUTHOR + ".lastname", TABLE_BOOK + ".authorid" };
		String strSortOrder = "title ASC";
		c = queryBuilder.query(mDatabase, asColumnsToReturn, null, null, null,
				null, strSortOrder);
		LogCursorInfo(c);
		c.close();

		// QUERY Again as above, but this time filter results to just titles
		// that contain the string "Prince"
		Log.i(DEBUG_TAG,
				"SQL QUERY EQUIVALENT: SELECT tbl_books.title, tbl_books.id, tbl_authors.firstname, tbl_authors.lastname, tbl_books.authorid FROM tbl_books INNER JOIN tbl_authors on tbl_books.authorid=tbl_authors.id  WHERE title LIKE '%Prince%' ORDER BY title ASC;");
		SQLiteQueryBuilder queryBuilder2 = new SQLiteQueryBuilder();
		queryBuilder2.setTables(TABLE_BOOK + ", " + TABLE_AUTHOR); // Tables to
																	// join
		queryBuilder2.appendWhere("(" + TABLE_BOOK + ".authorid" + "="
				+ TABLE_AUTHOR + ".id" + ")"); // how to join
		queryBuilder2.appendWhere(" AND (" + TABLE_BOOK + ".title"
				+ " LIKE '%Prince%'" + ")"); // WHERE clauses are AND'ed
												// together, so now we are
												// getting all titles with 'of'
		c = queryBuilder2.query(mDatabase, asColumnsToReturn, null, null, null,
				null, strSortOrder);
		LogCursorInfo(c);
		c.close();

		// Tip: Unions with QueryBuilder are overly complicated for simple
		// unions, just use raw queries with args.
		// Note: the Query args will all be surrounded with '' so you need to
		// put the % inside the arg instead of in the query string
		Log.i(DEBUG_TAG,
				"SQL QUERY EQUIVALENT: SELECT title AS Name, 'tbl_books' AS OriginalTable from tbl_books WHERE Name LIKE '%ow%' UNION SELECT (firstname||' '|| lastname) AS Name, 'tbl_authors' AS OriginalTable from tbl_authors WHERE Name LIKE '%ow%' ORDER BY Name ASC;");
		...
		c = mDatabase
				.rawQuery(sqlUnionExample, new String[] { "%ow%", "%ow%" });
		LogCursorInfo(c);
		c.close();

		// SIMPLE QUERY: select * from tbl_books WHERE id=9;
		Log.i(DEBUG_TAG,
				"SQL QUERY EQUIVALENT: select * from tbl_books WHERE id=9");
		c = mDatabase.query(TABLE_BOOK, null, "id=?", new String[] { "9" },
				null, null, null);
		LogCursorInfo(c);
		c.close();
		...
		updateBookTitle("The Little Prince", 9); // Change Book #9 to the
													// English title from the
													// French Version
		...
		// Deletes fail if they violate the database constraints we imposed in
		// the CREATE TABLE (triggers)
		try {
			Log.i(DEBUG_TAG,
					"Try to delete Stephen Colbert (Author Record Id 2)");
			deleteAuthor(2); // try to delete Colbert. This will FAIL because
								// he's got books (constraint violation)
		} catch (SQLiteConstraintException e) {
			Log.i(DEBUG_TAG,
					"Delete Failed, threw SQLiteConstraintException: "
							+ e.getMessage()
							+ "  ***FYI: Error Code 19 is Constraint Violated. Colbert still has books!");
		} catch (Exception e) {
			Log.i(DEBUG_TAG,
					"Delete failed; threw unexpected exception (perhaps a commit failed)."
							+ "[" + e.getLocalizedMessage() + "] ["
							+ e.getCause() + "]");
		}

		// DELETES DONE IN THE RIGHT ORDER TO AVOID CONSTRAINT VIOLATIONS WILL
		// SUCCEED
		Log.i(DEBUG_TAG,
				"Try to delete Stephen Colbert's Book (Book Record Id 8)");
		deleteBook(8); // Delete Colbert's book
		Log.i(DEBUG_TAG,
				"Try to delete Stephen Colbert Again (Author Record Id 2)");
		deleteAuthor(2); // Delete Colbert

		Log.i(DEBUG_TAG,
				"Deleting All books by J.K. Rowling (Author Record Id 1)");
	...
	// In this case, we are just logging so we treat all columns as strings
	...
	// We use a transaction here. The transaction is not required, but it can be
	// helpful
	// if you want to be able to rollback changes if something goes wrong in the
	// middle of a bunch
	// of database changes.
	// For example, there's no point in adding Books if their Author record
	...
	public void addSomeBooks() {
		...
		try {
			...
			Log.i(DEBUG_TAG,
					"Database Transaction?  " + mDatabase.inTransaction());
			Log.i(DEBUG_TAG,
					"Database Locked by current thread?  "
							+ mDatabase.isDbLockedByCurrentThread());
			...
			addAuthor(author); // FYI - this sets the ID appropriately, which is
								// then used by the addBook calls
			...
			// commit our changes, but roll them back, by NOT calling
	...
	public void addBook(Book newBook) {
		...
		Log.i(DEBUG_TAG, "Added Book:  " + newBook.mTitle + "(ID="
				+ newBook.mBookId + ")");
	}
	...
	public void addAuthor(Author newAuthor) {
		...
		newAuthor.mAuthorId = mDatabase.insertOrThrow(TABLE_AUTHOR, null,
				values);
		Log.i(DEBUG_TAG, "Added Author:  " + newAuthor.mFirstName + " "
				+ newAuthor.mLastName + "(ID=" + newAuthor.mAuthorId + ")");
	}
	...
	public void updateBookTitle(String newtitle, Integer bookId) {
		...
		mDatabase.update(TABLE_BOOK, values, "id=?",
	...
	public void deleteBook(Integer bookId) {
		mDatabase
				.delete(TABLE_BOOK, "id=?", new String[] { bookId.toString() });
	...
	public void deleteBooksByAuthor(Integer authorID) {
		int numBooksDeleted = mDatabase.delete(TABLE_BOOK, "authorid=?",
				new String[] { authorID.toString() });
		Log.i(DEBUG_TAG, "Deleted " + numBooksDeleted
				+ " books with Author Id:  " + authorID.toString());
	}

	// Delete a single author by its id
	public void deleteAuthor(Integer authorId) {
		mDatabase.delete(TABLE_AUTHOR, "id=?",
				new String[] { authorId.toString() });
